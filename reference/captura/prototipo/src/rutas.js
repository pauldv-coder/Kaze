...
export const PUERTOS = { E: 0, S: 1, O: 2, N: 3 };
...
const uniq = arr => Array.from(new Set(arr.map(v => Math.round(v)))).sort((a, b) => a - b);

/* Punto de anclaje de un lado de una forma (rect o rombo/círculo: mismo punto medio del lado). */
export function puntoPuerto(r, lado) {
...
/* obstaculos: [{x,y,w,h,id}] ya con su margen. canales: { xs:[], ys:[] } líneas preferidas.
   area: {x1,y1,x2,y2}. */
export function crearRuteador({ obstaculos, canales, area, margen = 12, quiebre = 36, compartido = 60 }) {
  const usados = new Map();
  const claveTramo = (x1, y1, x2, y2) => (x1 < x2 || (x1 === x2 && y1 < y2)) ? x1 + ',' + y1 + ',' + x2 + ',' + y2 : x2 + ',' + y2 + ',' + x1 + ',' + y1;
  // Índice espacial: celdas de 120 px con los obstáculos que las tocan.
  const CEL = 120;
  const celda = (cx, cy) => (cx + 2048) * 8192 + (cy + 2048);
  ...
  function ruta(desde, hasta, op) {
    op = op || {};
    let mejor = null;
    const ladosD = desde.lados || ['E', 'S', 'N'];
    const ladosH = hasta.linea != null ? ['N'] : (hasta.lados || ['O', 'N', 'S']);
    const combos = [];
    ladosD.forEach(ld => ladosH.forEach(lh => {
      const a = puntoPuerto(desde.rect, ld);
      const b = hasta.linea != null ? { x: a.x, y: hasta.linea } : puntoPuerto(hasta.rect, lh);
    ...
    for (const c of combos) {
      ...
      const r = intento(desde, c.ld, hasta, c.lh, op, mejor ? mejor.costo : Infinity);
      if (r && (!mejor || r.costo < mejor.costo)) mejor = r;
    }
    if (!mejor) return null;
    mejor.puntos.forEach((p, i) => { if (i) { const q = mejor.puntos[i - 1]; const k = claveTramo(q.x, q.y, p.x, p.y); usados.set(k, (usados.get(k) || 0) + 1); } });
    return mejor.puntos;
  }

  function intento(desde, ld, hasta, lh, op, tope) {
    const p0 = puntoPuerto(desde.rect, ld);
    const d0 = PUERTOS[ld];
    const s = { x: p0.x + DIRS[d0][0] * margen, y: p0.y + DIRS[d0][1] * margen };
    let pF, dF, t;
    if (hasta.linea != null) { t = null; dF = 1; }
    else {
    ...
    for (const holgura of [240, 100000]) {
      const r = buscar(s, d0, t, dF, hasta.linea, holgura, op, tope);
      if (r) {
  ...
  function buscar(s, d0, t, dF, linea, holgura, op, tope) {
    ...
    const minY = Math.max(area.y1, Math.min(s.y, t ? t.y : yMeta) - holgura), maxY = Math.min(yMeta != null ? Math.max(area.y2, yMeta) : area.y2, Math.max(s.y, t ? t.y : yMeta) + holgura);
    const enV = (v, a, b) => v >= a - 0.5 && v <= b + 0.5;
    ...
    const tx = t ? ix.get(Math.round(t.x)) : null, ty = t ? iy.get(Math.round(t.y)) : iy.get(Math.round(yMeta));
    ...
    const libreNodo = (i, j) => { const k = j * W + i; if (!nodoE[k]) nodoE[k] = dentro(xs[i], ys[j]) ? 2 : 1; return nodoE[k] === 1; };
    const libreH = (i, j) => { const k = j * W + i; if (!horE[k]) horE[k] = dentro((xs[i] + xs[i + 1]) / 2, ys[j]) ? 2 : 1; return horE[k] === 1; }; // tramo (i,j)→(i+1,j)
    const libreV = (i, j) => { const k = j * W + i; if (!verE[k]) verE[k] = dentro(xs[i], (ys[j] + ys[j + 1]) / 2) ? 2 : 1; return verE[k] === 1; }; // tramo (i,j)→(i,j+1)
    ...
    const h = (i, j) => (t ? Math.abs(xs[i] - t.x) + Math.abs(ys[j] - t.y) : Math.abs(ys[j] - yMeta));
    const N4 = W * H * 4;
...
export function simplificar(pts) {
  const r = [];
  pts.forEach(p => {
...
export function cruces(puntos, formas, ignorar) {
  const out = [];
