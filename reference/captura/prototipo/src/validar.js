...
import { secuencia, tieneValor, TIPOS_EVENTO } from './model.js';
import { cruces } from './rutas.js';

const limpio = s => String(s).replace(/[^A-Za-z0-9_]/g, '_');
const plural = (n, uno, varios) => n + ' ' + (n === 1 ? uno : varios);

export function revisarDiagrama(proc, m, gen) {
  const errores = [], avisos = [];
  const seq = secuencia(m);
  const enFase = seq.filter(s => s.numero != null);
  const info = {}; enFase.forEach(s => { info[s.clave] = s; });
  const cod = k => (info[k] ? info[k].codigo : '«' + ((m.actividades[k] || {}).nombre || 'actividad') + '»');
  const err = (texto, ir, ids) => errores.push({ texto, ir, ids: ids || [] });
  const av = (texto, ir, ids) => avisos.push({ texto, ir, ids: ids || [] });
  const irA = k => ({ tab: 'actividades', sub: 'caracterizar', clave: k });
  ...
  /* ---------- el flujo dibujado ---------- */
  if (gen && gen.geo && gen.xml) {
    const con = gen.geo.conectores || {};
    const flujos = Object.keys(con).map(id => Object.assign({ id }, con[id]));
    ...
    flujos.forEach(f => {
      ...
      const ir = r && r.tipo === 'salida' ? (() => { const d = m.decisiones.find(x => x.clave === r.decision); return d && d.origen ? irA(d.origen) : { tab: 'diagrama' }; })() : { tab: 'diagrama' };
