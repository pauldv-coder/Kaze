...
const bajo = s => String(s || '').toLowerCase();
...
const E_IE = new Set(['cerrar', 'encerrar', 'empezar', 'comenzar', 'recomendar', 'encomendar', 'enmendar', 'gobernar', 'pensar', 'repensar',
...
const O_UE = new Set(['costar', 'recostar', 'acostar', 'rodar', 'soltar', 'sonar', 'resonar', 'soñar', 'tronar', 'volar', 'sobrevolar', 'colgar',
...
const E_I = new Set(['medir', 'corregir', 'elegir', 'reelegir', 'regir', 'repetir', 'competir', 'rendir', 'derretir', 'concebir', 'gemir', 'colegir',
...
const IAR_TILDE = new Set(['enviar', 'reenviar', 'confiar', 'desconfiar', 'fiar', 'guiar', 'ampliar', 'variar', 'desviar', 'vaciar', 'criar',
...
const TILDE_RAIZ = { reunir: 'reúne', prohibir: 'prohíbe', rehusar: 'rehúsa', aislar: 'aísla', aunar: 'aúna', aullar: 'aúlla', maullar: 'maúlla',
...
const NO_VERBOS = new Set(['lugar', 'hogar', 'mujer', 'placer', 'ayer', 'par', 'bar', 'mar', 'militar', 'familiar', 'similar', 'popular', 'regular',
...
const VERBO_SI_SIGUE_OBJETO = new Set(['circular', 'regular', 'titular', 'auxiliar', 'ser', 'deber', 'poder', 'haber', 'saber', 'tener', 'parecer', 'desayunar']);

const PREFIJOS_TODOS = FAMILIAS.map(([base, forma, prefijos]) => ({ base, forma, prefijos }));

function cambiarRaiz(raiz, de, a) {
...
export function infinitivo(palabra, siguiente) {
...
const SING_S = new Set(['mes', 'gas', 'bus', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'virus', 'caos', 'cosmos', 'atlas', 'ómnibus',
...
const MASC_A = new Set(['día', 'mapa', 'planeta', 'cometa', 'sofá', 'tranvía', 'papá', 'mediodía', 'clima', 'idioma', 'problema', 'sistema', 'tema',
...
const FEM_O = new Set(['mano', 'foto', 'moto', 'radio', 'libido', 'seo', 'nao', 'dinamo', 'polio', 'metro', 'disco']);  // metro/disco: se corrigen abajo
FEM_O.delete('metro'); FEM_O.delete('disco');
const FEM_E = new Set(['llave', 'clave', 'calle', 'leche', 'noche', 'tarde', 'fuente', 'gente', 'mente', 'muerte', 'nieve', 'suerte', 'parte', 'base',
...
const FEM_CONSONANTE = new Set(['flor', 'labor', 'mujer', 'coliflor', 'sor', 'piel', 'miel', 'hiel', 'sal', 'cal', 'señal', 'catedral', 'credencial',
...
const MASC_CONSONANTE = new Set(['corazón', 'buzón', 'tazón', 'calzón', 'garzón', 'lápiz', 'pez', 'maíz', 'arroz', 'disfraz', 'tapiz', 'barniz',
...
const EL_FEM = new Set(['área', 'acta', 'agua', 'aula', 'alma', 'arma', 'alza', 'ala', 'ancla', 'hambre', 'habla', 'hacha', 'hada', 'ave', 'águila',
  'arca', 'asa', 'aura', 'haba', 'alta', 'aria', 'ánfora', 'ánima', 'álgebra', 'árnica', 'hampa', 'asma']);

const sinT = set => new Set(Array.from(set).map(quitarTildes));
...
const en = (set, setSt, w) => set.has(w) || setSt.has(quitarTildes(w));
// -ista: roles (el analista) frente a cosas (la lista, la entrevista).
const ISTAS_COSA = new Set(['lista', 'revista', 'vista', 'pista', 'arista', 'conquista', 'entrevista', 'autopista', 'checklist']);
// Siglas que son sustantivos comunes y llevan artículo: «el PDF», «la URL».
export const SIGLAS_COMUNES = { pdf: 'm', nit: 'm', rut: 'm', iva: 'm', csv: 'm', xml: 'm', url: 'f', api: 'f', kpi: 'm', sku: 'm', oc: 'f', op: 'f',
...
export function generoNumero(palabra, modo) {
...
const DET = new Set(['el', 'la', 'los', 'las', 'un', 'una', 'unos', 'unas', 'lo', 'al', 'del', 'este', 'esta', 'estos', 'estas', 'ese', 'esa', 'esos',
...
const PREP = new Set(['a', 'ante', 'bajo', 'con', 'contra', 'de', 'desde', 'durante', 'en', 'entre', 'hacia', 'hasta', 'mediante', 'para', 'por',
...
const MARCAS = new Set(['excel', 'word', 'powerpoint', 'outlook', 'teams', 'sharepoint', 'onedrive', 'drive', 'gmail', 'whatsapp', 'slack', 'jira',
...
const SUST_COMUNES_HERRAMIENTA = new Set(['correo', 'teléfono', 'telefono', 'portal', 'sistema', 'formato', 'plantilla', 'archivo', 'carpeta', 'hoja',
...
const ROLES = new Set(['jefe', 'jefa', 'auxiliar', 'analista', 'gerente', 'director', 'directora', 'coordinador', 'coordinadora', 'asistente',
...
const ACRONIMO_ROL = { ceo: 'el', cfo: 'el', coo: 'el', cto: 'el', cio: 'el', cmo: 'el', ciso: 'el', dpo: 'el', pmo: 'la', rrhh: '', ti: '' };
const CONECTORES_NOMBRE = new Set(['de', 'del', 'la', 'las', 'los', 'el', 'y', 'e', 'en', 'para', 'por', 'a', 'al', 'con']);

export const esDeterminante = w => DET.has(bajo(w));
const esAcronimo = w => /^[A-ZÁÉÍÓÚÑ0-9]{2,}s?$/.test(w) && /[A-ZÁÉÍÓÚÑ]{2}/.test(w);
const mayusInterna = w => /^.[^A-ZÁÉÍÓÚÑ]*[A-ZÁÉÍÓÚÑ]/.test(w) && !esAcronimo(w) && /[a-záéíóúñ]/.test(w);
const capitalizada = w => /^[A-ZÁÉÍÓÚÑ]/.test(w);

/* ¿El texto es un nombre propio (una persona, una marca, un sistema con nombre)? */
export function esNombrePropio(texto, modo) {
...
export function minus1(texto, modo) {
...
export const mayus1 = t => { const s = String(t || ''); return s.charAt(0).toUpperCase() + s.slice(1); };
export const sinPunto = t => String(t || '').trim().replace(/[.;:]+$/, '').trim();
export const oracion = t => { const s = sinPunto(t); return s ? mayus1(s) + (/[?!»"”)]$/.test(s) ? '' : '.') : ''; };
export const cita = t => '«' + sinPunto(t).replace(/^[«"“]+|[»"”]+$/g, '') + '»';
...
export function conArticulo(texto, modo) {
...
export function conPrep(prep, frase) {
...
export function lista(items, conj) {
...
export function elementos(texto) {
  const t = String(texto || '').trim();
  if (!t) return [];
...
/* ---------- oraciones con verbo ---------- */
const VERBOS_COMUNES = new Set(['es', 'está', 'están', 'son', 'hay', 'queda', 'quedan', 'llega', 'llegan', 'tiene', 'tienen', 'publica', 'publican',
...
const DET_TRAS_VERBO = new Set(['el', 'la', 'los', 'las', 'un', 'una', 'unos', 'unas', 'su', 'sus', 'este', 'esta', 'estos', 'estas', 'ese', 'esa',
  'cada', 'todo', 'toda', 'todos', 'todas', 'lo']);
export function esClausula(texto) {
...
const SIN_ARTICULO = { hacer: ['seguimiento', 'clic', 'click', 'uso', 'caso', 'falta', 'parte', 'efectivo', 'entrega', 'cierre'], dar: ['respuesta', 'aviso', 'clic', 'click', 'trámite', 'curso', 'visto', 'alcance', 'cumplimiento', 'fe', 'traslado', 'lugar'],
...
export function accionDe(nombre, plural) {
...
const RIGE_INFINITIVO = new Set(['para', 'sin', 'de', 'a', 'al', 'por', 'hasta', 'antes', 'después', 'tras', 'que', 'puede', 'pueden', 'debe', 'deben', 'suele', 'suelen', 'va', 'van']);
function coordinadas(texto, plural) {
...
export function aTercera(texto, plural) {
...
/* ---------- tiempos ---------- */
const NUM_TXT = { 1: 'una', 2: 'dos', 3: 'tres', 4: 'cuatro', 5: 'cinco', 6: 'seis', 7: 'siete', 8: 'ocho', 9: 'nueve', 10: 'diez' };
...
export function duracion(n, unidad) {
